...
func modAllowSubCmnd(s *discordgo.Session, m *discordgo.MessageCreate, a BotArgs,
	c ifaces.IConfigurator, cmd *CommandRegistrant) (*CommandOutput, ICommandError) {

	var (
		out     = newCommandOutput(cmd, "Allow Clientside Mods")
		failed  = make([]string, 0)
		reason  = make([]string, 0)
		success = make([]int64, 0)
	)
	...
	for _, mod := range a[2:] {
		if id, err := strconv.ParseInt(mod, 10, 64); err == nil {
			if err := c.AddClientMod(id); err != nil {
				failed = append(failed, mod)
				reason = append(reason, err.Error())
			} else {
				logger.LogInfo(cmd, sprintf("%s added %d to the mod configuration",
...
func modDisallowSubCmnd(s *discordgo.Session, m *discordgo.MessageCreate, a BotArgs,
	c ifaces.IConfigurator, cmd *CommandRegistrant) (*CommandOutput, ICommandError) {
	var (
		out     = newCommandOutput(cmd, "Remove Clientside Mods")
		failed  = make([]string, 0)
		reason  = make([]string, 0)
		success = make([]int64, 0)
	)
	...
	for _, mod := range a[2:] {
		if id, err := strconv.ParseInt(mod, 10, 64); err == nil {
			if err := c.RemoveClientMod(id); err != nil {
				failed = append(failed, mod)
				reason = append(reason, err.Error())
			} else {
				logger.LogInfo(cmd, sprintf("> %s removed %d from the mod configuration",
...
func listModsSubCmnd(s *discordgo.Session, m *discordgo.MessageCreate, a BotArgs,
	...
	if len(servermods) < 1 {
		...
		for i, id := range servermods {
			out.AddLine(sprintf("> %d. %s%d", i+1, modURLBase, id))
		}
	}

	if len(clientmods) > 0 {
		out.AddLine("**Mods Allowed**")
		for i, id := range clientmods {
			out.AddLine(sprintf("> %d. %s%d", i+1, modURLBase, id))
		}
	}
